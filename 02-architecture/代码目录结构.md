# XTrendAI 代码目录结构 v2.1

## 1. 项目根目录结构

```
XTrendAI/
├── app/                          # Next.js App Router (页面+API)
│   ├── [locale]/                 # 国际化路由
│   │   ├── (main)/               # 主页面组 (不含布局前缀)
│   │   │   ├── page.tsx          # 首页 (/ 或 /zh-CN 或 /en-US)
│   │   │   ├── profile/          # 用户配置页面
│   │   │   │   └── page.tsx      # /profile
│   │   │   ├── results/          # 热点结果页面
│   │   │   │   └── page.tsx      # /results
│   │   │   └── layout.tsx        # 主页面组布局
│   │   └── api/                  # API路由 (不需要locale前缀)
│   │       ├── trends/           # 热点相关API
│   │       │   └── route.ts      # GET /api/trends
│   │       ├── tasks/            # 任务相关API
│   │       │   └── route.ts      # POST /api/tasks
│   │       ├── user/             # 用户相关API
│   │       │   ├── profile/route.ts  # GET/PUT /api/user/profile
│   │       │   └── quota/route.ts    # GET /api/user/quota
│   │       ├── auth/             # 认证相关API
│   │       │   └── route.ts      # POST /api/auth/login
│   │       └── i18n/             # 国际化相关API
│   │           └── route.ts      # POST /api/i18n/language
│   ├── layout.tsx                # 根布局
│   └── globals.css               # 全局样式
│
├── components/                   # React组件
│   ├── ui/                       # shadcn/ui基础组件
│   │   ├── button.tsx
│   │   ├── card.tsx
│   │   ├── dialog.tsx
│   │   ├── select.tsx
│   │   └── ...
│   ├── layout/                   # 布局组件
│   │   ├── Header.tsx            # 顶部导航栏
│   │   ├── Footer.tsx            # 底部
│   │   ├── LanguageSelector.tsx  # 语言切换器
│   │   └── Sidebar.tsx           # 侧边栏(可选)
│   ├── hotspot/                  # 热点相关组件
│   │   ├── HotspotList.tsx       # 热点列表
│   │   ├── HotspotCard.tsx       # 单个热点卡片
│   │   ├── TaskList.tsx          # 任务列表
│   │   ├── TaskCard.tsx          # 单个任务卡片
│   │   └── CopyButton.tsx        # 复制按钮
│   ├── profile/                  # 用户配置组件
│   │   ├── ProfileForm.tsx       # 配置表单
│   │   ├── ProfileStepper.tsx    # 配置向导
│   │   ├── LanguageStep.tsx      # 语言选择步骤
│   │   ├── RegionSelector.tsx    # 地区选择器
│   │   ├── AgeSelector.tsx       # 年龄选择器
│   │   └── ScenarioSelector.tsx  # 场景选择器
│   └── common/                   # 通用组件
│       ├── LoadingSpinner.tsx    # 加载动画
│       ├── ErrorMessage.tsx      # 错误提示
│       ├── EmptyState.tsx        # 空状态
│       └── Toast.tsx             # 提示消息
│
├── lib/                          # 工具函数和配置
│   ├── utils.ts                  # 通用工具函数
│   ├── constants.ts              # 常量定义
│   ├── validation.ts             # 数据验证schema (Zod)
│   └── format.ts                 # 格式化函数
│
├── locales/                      # 国际化翻译资源 (新增)
│   ├── zh-CN.json                # 中文翻译
│   │   {
│   │     "common": {...},
│   │     "home": {...},
│   │     "profile": {...},
│   │     "templates": {...}  # 任务模板
│   │   }
│   └── en-US.json                # 英文翻译
│       {
│         "common": {...},
│         "home": {...},
│         "profile": {...},
│         "templates": {...}  # Task templates
│       }
│
├── services/                     # 业务服务层
│   ├── TrendService.ts           # 热点采集服务
│   ├── FilterService.ts          # 筛选服务
│   ├── TaskService.ts            # 任务生成服务
│   ├── UserService.ts            # 用户服务
│   ├── QuotaService.ts           # 配额管理服务
│   ├── AuthService.ts            # 认证服务
│   ├── TemplateService.ts        # 模板管理服务
│   └── I18nService.ts            # 国际化服务 (新增)
│
├── stores/                       # 数据存储层 (MVP阶段)
│   ├── UserStore.ts              # 用户数据存储
│   ├── TrendStore.ts             # 热点数据存储
│   ├── TemplateStore.ts          # 模板存储
│   └── QuotaStore.ts             # 配额数据存储
│
├── types/                        # TypeScript类型定义
│   ├── user.ts                   # 用户相关类型
│   ├── trend.ts                  # 热点相关类型
│   ├── task.ts                   # 任务相关类型
│   ├── i18n.ts                   # 国际化相关类型 (新增)
│   ├── api.ts                    # API请求/响应类型
│   └── index.ts                  # 统一导出
│
├── data/                         # 静态数据 (MVP阶段)
│   ├── mock-trends.json          # 模拟热点数据
│   ├── templates.json            # 任务模板
│   └── regions.json              # 地区配置
│
├── hooks/                        # 自定义React Hooks
│   ├── useTrends.ts              # 热点数据Hook
│   ├── useUser.ts                # 用户数据Hook
│   ├── useQuota.ts               # 配额Hook
│   ├── useCopy.ts                # 复制功能Hook
│   ├── useToast.ts               # 提示消息Hook
│   └── useI18n.ts                # 国际化Hook (新增)
│
├── middleware/                   # 中间件
│   ├── auth.ts                   # 鉴权中间件
│   ├── quota.ts                  # 配额检查中间件
│   ├── i18n.ts                   # 国际化中间件 (新增)
│   └── logger.ts                 # 日志中间件
│
├── config/                       # 配置文件
│   ├── site.ts                   # 网站配置
│   ├── i18n.ts                   # 国际化配置 (新增)
│   ├── scenarios.ts              # 场景配置
│   └── quotas.ts                 # 配额配置
│
├── public/                       # 静态资源
│   ├── icons/                    # 图标
│   ├── images/                   # 图片
│   └── favicon.ico
│
├── .env.local                    # 本地环境变量
├── .env.example                  # 环境变量示例
├── .gitignore                    # Git忽略文件
├── next.config.js                # Next.js配置 (含i18n配置)
├── tailwind.config.ts            # Tailwind配置
├── tsconfig.json                 # TypeScript配置
├── package.json                  # 项目依赖
└── README.md                     # 项目说明
```

---

## 2. 核心目录说明

### 2.1 app/[locale]/ - 国际化路由

采用Next.js 15的动态路由实现国际化：

```
app/
├── [locale]/           # 动态语言路由
│   ├── page.tsx        # / → 自动重定向到 /zh-CN 或 /en-US
│   ├── profile/        # /zh-CN/profile, /en-US/profile
│   └── api/            # API不需要locale前缀
│       └── ...
└── layout.tsx          # 根布局，处理语言检测和重定向
```

**路由映射**:

| 访问URL | 实际渲染 | 说明 |
|---------|---------|------|
| `/` | 重定向 | 根据浏览器语言重定向 |
| `/zh-CN` | 中文首页 | 中文界面 |
| `/en-US` | 英文首页 | 英文界面 |
| `/api/*` | API路由 | 不受国际化影响 |

### 2.2 locales/ - 翻译资源

翻译文件结构示例：

```json
// locales/zh-CN.json
{
  "common": {
    "appName": "XTrendAI",
    "loading": "加载中...",
    "error": "出错了",
    "copy": "复制",
    "copied": "已复制"
  },
  "home": {
    "title": "今日热点",
    "fetchTrends": "获取最新热点",
    "lastUpdate": "最后更新"
  },
  "profile": {
    "title": "用户配置",
    "steps": {
      "language": "选择界面语言",
      "region": "选择您关注的地区",
      "age": "选择您的年龄段",
      "ethnicity": "选择您的族裔",
      "scenarios": "选择您想要的场景"
    }
  },
  "templates": {
    "POD": {
      "tshirt": "一款适合{trendName}的潮流T恤设计，{style}，{specs}"
    }
  }
}
```

```json
// locales/en-US.json
{
  "common": {
    "appName": "XTrendAI",
    "loading": "Loading...",
    "error": "Error occurred",
    "copy": "Copy",
    "copied": "Copied"
  },
  "home": {
    "title": "Today's Trends",
    "fetchTrends": "Get Latest Trends",
    "lastUpdate": "Last Updated"
  },
  "profile": {
    "title": "User Profile",
    "steps": {
      "language": "Select Interface Language",
      "region": "Select Your Region",
      "age": "Select Your Age Group",
      "ethnicity": "Select Your Ethnicity",
      "scenarios": "Select Scenarios"
    }
  },
  "templates": {
    "POD": {
      "tshirt": "A trendy t-shirt design for {trendName}, {style}, {specs}"
    }
  }
}
```

### 2.3 components/layout/LanguageSelector.tsx

语言切换组件：

```typescript
// components/layout/LanguageSelector.tsx
export function LanguageSelector() {
  const { locale, setLocale } = useI18n()

  return (
    <Select value={locale} onValueChange={setLocale}>
      <SelectTrigger>
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        <SelectItem value="zh-CN">🇨🇳 中文</SelectItem>
        <SelectItem value="en-US">🇺🇸 English</SelectItem>
      </SelectContent>
    </Select>
  )
}
```

### 2.4 services/I18nService.ts

国际化服务：

```typescript
// services/I18nService.ts
export class I18nService {
  /**
   * 检测请求语言
   */
  static detectLanguage(request: Request): string {
    // 1. 检查Cookie
    const cookieLang = this.getLanguageFromCookie(request)
    if (cookieLang) return cookieLang

    // 2. 检查Accept-Language头
    const acceptLang = this.getLanguageFromHeader(request)
    if (acceptLang) return acceptLang

    // 3. 返回默认语言
    return 'zh-CN'
  }

  /**
   * 获取任务输出语言（混合策略）
   */
  static getOutputLanguage(
    scenario: Scenario,
    interfaceLang: string
  ): string {
    // POD场景始终使用英文
    if (scenario === 'POD') return 'en-US'

    // 内容创作使用界面语言
    if (scenario === 'CONTENT') return interfaceLang

    // 营销场景根据目标市场
    if (scenario === 'MARKETING') {
      // 可以根据用户选择的地区决定
      return interfaceLang
    }

    return interfaceLang
  }

  /**
   * 获取翻译后的任务模板
   */
  static getTaskTemplate(
    scenario: string,
    taskType: string,
    outputLang: string
  ): string {
    const templates = outputLang === 'zh-CN'
      ? zhCNTranslations.templates
      : enUSTranslations.templates

    return templates[scenario][taskType]
  }
}
```

### 2.5 hooks/useI18n.ts

国际化Hook：

```typescript
// hooks/useI18n.ts
export function useI18n() {
  const [locale, setLocale] = useState<string>('zh-CN')

  const t = (key: string, params?: Record<string, string>) => {
    // 获取翻译并替换参数
    return getTranslation(key, locale, params)
  }

  const switchLanguage = (newLocale: string) => {
    setLocale(newLocale)
    document.cookie = `lang=${newLocale};path=/;max-age=31536000`
    // 刷新页面或更新路由
  }

  return {
    locale,
    setLocale: switchLanguage,
    t
  }
}
```

### 2.6 config/i18n.ts

国际化配置：

```typescript
// config/i18n.ts
export const i18nConfig = {
  // 默认语言
  defaultLocale: 'zh-CN',

  // 支持的语言
  locales: ['zh-CN', 'en-US'] as const,

  // 语言名称映射
  localeNames: {
    'zh-CN': '简体中文',
    'en-US': 'English'
  },

  // 语言标志
  localeFlags: {
    'zh-CN': '🇨🇳',
    'en-US': '🇺🇸'
  }
} as const

export type Locale = typeof i18nConfig.locales[number]
```

---

## 3. 文件命名规范

### 3.1 组件文件

| 规则 | 示例 | 说明 |
|-----|------|------|
| PascalCase | `HotspotCard.tsx` | React组件 |
| kebab-case目录 | `hotspot/` | 组件目录 |
| index.ts导出 | `components/index.ts` | 统一导出入口 |

### 3.2 工具/服务文件

| 规则 | 示例 | 说明 |
|-----|------|------|
| PascalCase + Service后缀 | `I18nService.ts` | 服务类 |
| camelCase | `utils.ts` | 工具函数 |
| kebab-case | `use-i18n.ts` | Hooks (也可用camelCase `useI18n.ts`) |

### 3.3 类型文件

| 规则 | 示例 | 说明 |
|-----|------|------|
| camelCase | `i18n.ts` | 类型定义文件 |

---

## 4. 导入组织规范

### 4.1 导入顺序

```typescript
// 1. 外部依赖
import { useState } from 'react'
import { Button } from '@/components/ui/button'

// 2. 国际化
import { useI18n } from '@/hooks/useI18n'
import { I18nService } from '@/services/I18nService'

// 3. 内部组件
import { HotspotCard } from '@/components/hotspot/HotspotCard'

// 4. 服务层
import { TrendService } from '@/services/TrendService'

// 5. 类型
import type { UserProfile, Locale } from '@/types'

// 6. 工具函数
import { formatNumber } from '@/lib/format'

// 7. CSS
import './HotspotList.css'
```

### 4.2 路径别名

```typescript
// tsconfig.json
{
  "compilerOptions": {
    "paths": {
      "@/*": ["./*"],
      "@/components/*": ["./components/*"],
      "@/lib/*": ["./lib/*"],
      "@/services/*": ["./services/*"],
      "@/types/*": ["./types/*"],
      "@/hooks/*": ["./hooks/*"],
      "@/data/*": ["./data/*"],
      "@/locales/*": ["./locales/*"],
      "@/config/*": ["./config/*"]
    }
  }
}
```

---

## 5. 代码组织原则

### 5.1 单一职责

每个文件/模块只负责一件事：
- 组件文件只包含UI逻辑
- 服务文件只包含业务逻辑
- 类型文件只包含类型定义
- 翻译文件只包含翻译内容

### 5.2 依赖方向

```
页面 (app/[locale]/)
  ↓ 依赖
组件 (components/)
  ↓ 依赖
Hooks (hooks/) + 服务 (services/)
  ↓ 依赖
存储 (stores/) + 类型 (types/) + 翻译 (locales/)
  ↓ 依赖
工具 (lib/) + 配置 (config/)
```

### 5.3 国际化资源组织

- 翻译文件按语言分离
- 任务模板包含在翻译文件中
- 支持参数插值 `{variable}`
- 支持嵌套命名空间

---

## 6. MVP简化结构 (V1.0)

对于8小时快速交付，可以简化部分目录：

```
简化版结构 (V1.0 MVP)
├── app/                    # 页面+API (含[locale]路由)
├── components/             # 组件 (完整)
├── services/               # 服务 (简化，合并部分逻辑)
├── lib/                    # 工具 (完整)
├── locales/                # 翻译资源 (完整) ← 新增
├── types/                  # 类型 (简化，合并到各文件)
├── data/                   # 静态数据 (完整)
├── hooks/                  # Hooks (简化)
├── config/                 # 配置 (完整)
└── stores/                 # 存储 (使用JSON文件)

暂不创建:
├── middleware/             # V1.0在API层简单处理，i18n在layout处理
├── styles/                 # 使用Tailwind
└── 复杂的缓存层            # V1.0使用简单内存缓存
```

---

## 7. 国际化开发注意事项

### 7.1 组件中使用翻译

```typescript
// ✅ 推荐：使用Hook
function HotspotCard() {
  const { t } = useI18n()
  return <div>{t('home.title')}</div>
}

// ✅ 服务端组件：使用服务端翻译函数
async function HotspotList() {
  const t = await getTranslations('home')
  return <div>{t('title')}</div>
}
```

### 7.2 添加新翻译步骤

1. 在 `locales/zh-CN.json` 添加中文翻译
2. 在 `locales/en-US.json` 添加英文翻译
3. 组件中使用 `t('key')` 引用

### 7.3 任务模板翻译

任务模板需要在两种语言中保持一致的参数结构：

```json
// zh-CN.json
"templates": {
  "POD": {
    "tshirt": "一款适合{trendName}的潮流T恤设计，{style}，{specs}"
  }
}

// en-US.json - 参数名必须一致
"templates": {
  "POD": {
    "tshirt": "A trendy t-shirt design for {trendName}, {style}, {specs}"
  }
}
```

---

*文档版本: v2.1*
*创建日期: 2026-02-26*
*最后更新: 2026-02-26 (新增国际化目录结构)*
*负责人: 系统架构师*
